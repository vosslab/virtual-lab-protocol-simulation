...
The legacy scene runtime at `scenes` is **frozen**. It exists
only as legacy reference and emergency-compatibility support during the migration
to `scene_runtime`. It receives no new behavior, no
...
Per `PRIMARY_CONTRACT.md`, shared behavior and runtime
...
These rules follow `REPO_STYLE.md` (long-term over
...
- New feature logic. New features land in
  `scene_runtime` and in declarative YAML, never
  in `src/scenes/`.
...
- `scene_runtime` - generic runtime, contract
