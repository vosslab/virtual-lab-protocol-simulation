...
- `dev_smoke` -- developer smoke test or fixture; lives under `tests/content/dev_smoke/`; exempt from step-count and learning-block gates.
...
- Acceptance criteria:
  - Every `protocol.yaml` under `content/` (including `dev_smoke/`) has an explicit `entry:` block.
