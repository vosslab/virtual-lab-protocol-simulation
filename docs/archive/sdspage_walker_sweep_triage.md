...
> RESOLVED 2026-07-05. Root cause: the sweep reused a leftover
> `python3 -m http.server --directory dist` (Playwright
> `reuseExistingServer: !CI`), so the walker was served a stale `dist/` bundle
> whose embedded `PROTOCOLS` snapshot predated the `sdspage_*` protocols. Not a
> content, scene, asset, or runtime defect -- a clean rebuild turned all 8 green
> (`86 passed`). Fixed by setting `reuseExistingServer: false` (every sweep
> rebuilds+serves fresh) plus correcting the dead `dist/main.js` gate in
> `run_playwright_tests.sh`. Both failure shapes (5 boot timeouts, 3
> console-error) traced to this one stale-bundle cause. This audit is closed;
> `git mv` it to `docs/archive/` (human owns git).
