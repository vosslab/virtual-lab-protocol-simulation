...
|--------|---------|-----|--------|
| `kimwipe_pad` | NO_VISUAL_STATES | Added `visible: bool` field with svg cases → `kimwipe_pad` | ✓ FIXED |
| `paper_towel_pad` | NO_VISUAL_STATES | Added `visible: bool` field with svg cases → `kimwipe_pad` | ✓ FIXED |
| `lens_tissue` | NO_VISUAL_STATES | Added `visible: bool` field with svg cases → `kimwipe_pad` | ✓ FIXED |
| `micropipette_tip_box` | NO_VISUAL_STATES | Added `visible: bool` field with svg cases → `tip_box` | ✓ FIXED |
| `cell_counter` | NO_SVG_STATES | Added `slide_loaded: svg` cases → `cell_counter` | ✓ FIXED |

### Bottle Objects (REMAP_POSSIBLE: generic `bottle`)

| Object | Problem | Fix | Result |
|--------|---------|-----|--------|
| `ethanol_bottle` | MISSING_ASSET: `ethanol_bottle` | Remap to `bottle` | ✓ FIXED |
| `media_bottle` | MISSING_ASSET: `media_bottle` | Remap to `bottle` | ✓ FIXED |
| `sterile_water_bottle` | MISSING_ASSET: `water_bottle` | Remap to `bottle` | ✓ FIXED |
| `carboplatin_stock_bottle` | MISSING_ASSET: `carboplatin_bottle` | Remap to `bottle` | ✓ FIXED |
| `metformin_stock_bottle` | MISSING_ASSET: `metformin_bottle` | Remap to `bottle` | ✓ FIXED |
| `dmso_bottle` | MISSING_ASSET: `dmso_bottle` | Remap to `bottle` | ✓ FIXED |

### Container/Tube Objects (REMAP_POSSIBLE: `falcon_15ml` / `microtube`)

| Object | Problem | Fix | Result |
|--------|---------|-----|--------|
| `conical_tube_for_dilution` | MISSING_ASSET: `conical_15ml` | Remap to `falcon_15ml` | ✓ FIXED |
| `microtube_15ml_intermediate` | MISSING_ASSET: `microtube_15ml` | Remap to `falcon_15ml` | ✓ FIXED |
| `metformin_working_tube` | MISSING_ASSET: `microtube_1ml` | Remap to `microtube` | ✓ FIXED |
| `conical_15ml_rack` (subparts) | MISSING_ASSET: `conical_15ml` | Remap to `falcon_15ml` + add rack svg | ✓ FIXED |

### Lab Equipment (REMAP_POSSIBLE: generic/existing)

| Object | Problem | Fix | Result |
|--------|---------|-----|--------|
| `hood_surface` | MISSING_ASSET: `hood_surface_dirty`/`clean` | Remap to `glove_box` | ✓ FIXED |
| `incubator` | MISSING_ASSET: `incubator_closed`/`open` | Remap to `incubator` (base) | ✓ FIXED |
| `label_pen` | MISSING_ASSET: `label_pen_idle`/`in_hand` | TEMPORARY SUBSTITUTE: `bottle` | ⚠️ NEEDS AUTHORING |
| `cell_counter` | NO_SVG_STATES (fixed above) | Added svg case → `cell_counter` | ✓ FIXED |
| `counter_slide_cartridge` | NO_SVG_STATES | Added material_name svg cases → `tip_box` | ✓ FIXED |
| `hemocytometer_slide` | NO_SVG_STATES | Added material_name svg cases → `bottle` | ✓ FIXED |

### Structured Objects (Plates/Racks/Gels)

| Object | Problem | Fix | Result |
|--------|---------|-----|--------|
| `gel_cassette` (subparts) | MISSING_ASSET: `gel_lane_*` | Remap subpart cases to `mini_protean_gel` + add cassette svg | ✓ FIXED |
| `microtube_rack_24` | NO_SVG_STATES | Added material_name svg → `microtube_rack_24_placeholder` | ✓ FIXED |

### Powder/Solution Tubes (REMAP_POSSIBLE: `mtt_vial`)

| Object | Problem | Fix | Result |
|--------|---------|-----|--------|
| `mtt_powder_container` | MISSING_ASSET: `mtt_powder_container` | Remap to `mtt_vial` | ✓ FIXED |
| `mtt_solution_tube` | MISSING_ASSET: `mtt_solution_tube` | Remap to `mtt_vial` | ✓ FIXED |
...
| Check | Status |
|-------|--------|
| `bash build_github_pages.sh` | ✓ PASS |
| `npx tsc --noEmit -p tsconfig.json` | ✓ PASS |
| Generated object_data.ts | ✓ OK (78 objects) |
| Protocol HTML files | ✓ OK (26 files) |
