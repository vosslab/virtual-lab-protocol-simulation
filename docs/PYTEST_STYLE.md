...
* Store tests in `tests/` with files named `test_*.py`. Note: Playwright browser checks (`.mjs` files) also live in `tests/`; pytest only collects `.py` files, so they coexist without slowing the suite-see [E2E_TESTS.md](E2E_TESTS.md).
