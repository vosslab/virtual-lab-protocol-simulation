...
- No cell may appear in more than one group within the same `group_kind`.
  The build pipeline validates that no two members of the same `group_kind`
  overlap.
- A `region` group kind is the escape hatch for "the whole object" and
  "a contiguous semantic block of cells". Region groups are bounded by the
  build-pipeline validation rule above (every contained cell must exist
  and belong to no other region); they are NOT an open authoring surface
  for arbitrary subcell collections.
