...
| dilution_tube_rack_8 | rack | content/objects/dilution_tube_rack_8.yaml | WS-OBJ-STRUCT | per-tube: cn (enum: empty, carboplatin, media), cv (float, unit=ul, max=1000, applies_to=subpart) | OBJECT_YAML_FORMAT.md structure (list layout) | DDS. 8 microtubes (B..H plus a parent slot) holding the carboplatin working dilution series; structure.layout=list, count=8, name_pattern "tube_{row_letter}". |
