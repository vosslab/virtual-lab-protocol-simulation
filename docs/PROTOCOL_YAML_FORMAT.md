...
| `scene` | string | yes | Where this step's interactions happen: `cell_culture_hood`, `bench`, `incubator`, `microscope`, `plate`, `plate_reader`, or `well_plate_workspace` |
...
| Field | Type | Description |
| --- | --- | --- |
| `scene` | string | Scene where the completion event fires (e.g. `cell_culture_hood`, `microscope`, `bench`) |
...
- `scene` is one of `cell_culture_hood | bench | incubator | microscope | plate | plate_reader | well_plate_workspace`.
...
- For a step with `scene: cell_culture_hood`, every non-virtual item must have `scene: cell_culture_hood`
