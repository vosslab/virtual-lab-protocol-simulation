...
def generate_protocol_data(steps, parts, days, scene_router='legacy', protocol_name='cell_culture'):
	...
	lines = [
		...
		'',
		f'export const SCENE_ROUTER: "legacy" | "driver" = "{scene_router}";',
		'',
...
def main():
	...
	# Extract and validate optional sceneRouter field (WP-F1.2)
	# Valid values: 'legacy' or 'driver'. Defaults to 'legacy' when absent.
	scene_router = protocol.get('sceneRouter', 'legacy')
	if scene_router not in ('legacy', 'driver'):
		print(f"Error: sceneRouter value '{scene_router}' not in ['legacy', 'driver']", file=sys.stderr)
		return 1
	...
	protocol_data_ts = generate_protocol_data(steps, parts, days, scene_router, args.protocol)
