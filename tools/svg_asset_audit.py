...
Usage:
	python3 tools/svg_asset_audit.py
	python3 tools/svg_asset_audit.py --object gel_cassette
	python3 tools/svg_asset_audit.py --format json
"""
...
# Insert repo root so shared_toolkit imports resolve
...
def print_full_report(
	...
	if verbose:
		print_provenance_section(per_asset_metadata)
		print()

		print_svg_health_section(per_asset_metadata)
		print()

		print_object_alignment_section(objects, asset_reuse)
		print()
...
def parse_args():
	...
	selection_group.add_argument(
		'-o', '--object',
		dest='object_name',
		help='Focus on one object (name match).'
	)
	selection_group.add_argument(
		'--list-objects',
	...
	selection_group.add_argument(
		...
		dest='interactive',
		action='store_true',
		help='Pick an object from a numbered menu.'
	)

	verbosity_group = parser.add_argument_group('Verbosity')
	verbosity_quiet = verbosity_group.add_mutually_exclusive_group()
	verbosity_quiet.add_argument(
		'-q', '--quiet',
		dest='quiet',
		action='store_true',
		help='Suppress section headers and decorative PASS lines. Findings + summary still print.'
	)
	verbosity_quiet.add_argument(
		'-v', '--verbose',
		dest='verbose',
		action='store_true',
		help='Print per-asset detail (full walk of all five sections). Default is summary-only.'
	)
	...
	output_group.add_argument(
		...
		choices=['table', 'json'],
		default='table',
		help='Output format: table (default) or json. JSON always emits full data regardless of verbosity.'
	)
