...
def list_protocols(protocols_dir=None):
	...
	"""
	root = protocols_dir if protocols_dir is not None else tools.shared_toolkit.paths.PROTOCOLS_DIR
	root_path = Path(root)
...
def resolve_protocol_path(name_or_path, repo_root=None):
	...
	"""
	root = repo_root if repo_root is not None else tools.shared_toolkit.paths.REPO_ROOT
