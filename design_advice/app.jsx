    ...
    <div className="page">
      ...
      <div className="yaml-block">
        ...
        <div className="preview-col">
          <div className="preview-title">Live preview * viewport 1920x1080</div>
          <SceneCanvas
            scene={scene}
            items={finalItems}
            showSceneBounds
      ...
      <Stage n="01" kicker="Stage 1 * Inputs"
        ...
        <StageGrid>
          <StageText>
            ...
            <p className="sub">Plus the runtime viewport size - usually <Pill>1920x1080</Pill>. The pipeline is pure: same inputs -> same pixels.</p>
          </StageText>
          <StageViz>
...
microtube_rack: {
  ...
  aspect:        1.55,
      ...
      <Stage n="05" kicker="Stage 5 * Scale to real-world dimensions"
             title="Apply the cm x px_per_cm sizing model, per SCALING_MODEL.md.">
        <StageGrid>
          <StageText>
...
plate_reader: 8.0 px/cm`}</pre>
            ...
            <pre className="mono small">{`width_scale = (display_width_cm x px_per_cm)
              / (default_width x 11.52)
...
// at 1280 viewport, 90% usable area`}</pre>
  ...
  visual_width x width_scale x depth_scale,
  label_width capped at footprint x 2.5
)`}</pre>
      ...
      <Stage n="10" kicker="Stage 10 * Scene-bounds clamp + render"
        ...
        <StageGrid>
          ...
          <StageViz title="Final rendered scene">
            ...
            />
            <div className="legend">
              <span className="lg-dot" style={{ background: '#FCFAF1', border: '1px solid #C9C3B5' }}/> viewport (1920x1080)
  ...
  return (
    <header className="hero">
      ...
      <p className="hero-sub">
        Eight pure stages turn an author-written YAML configuration into a rendered scene at 1920x1080.
...
function PipelineDiagram() {
  const stages = [
    ...
    { n: "05", t: "Scale", d: "cm x px_per_cm -> width_scale" },
            ...
            {(normalScene?.zones || []).map((z, i) => (
              <div key={i} className="sn-resolved-card">
                ...
                <span className="mono small">
                  {z.bounds.left},{z.bounds.top} * {z.bounds.right - z.bounds.left}x{z.bounds.bottom - z.bounds.top}
                </span>
...
// Scale table - for each placement, show display_width_cm x px_per_cm /
// (default_width x 11.52) -> _width_scale. Highlights the cm-model rows.
function ScaleTable({ scaled }) {
  if (!scaled.length) return <div className="empty">no placements parsed</div>;
  return (
    <DataTable
      headers={["placement_name", "display_cm", "x px/cm", "/ default x 11.52", "= _width_scale", "source"]}
      rows={scaled.map(p => {
