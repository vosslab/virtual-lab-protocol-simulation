...
| `PX_PER_SCENE_PERCENT` | `11.52` | Empirical: 1280-px viewport x 90% usable / 100. Used by Stage 5 scaling math. |
...
```
_width_scale = (display_width_cm x px_per_cm)
               / (default_width x PX_PER_SCENE_PERCENT)
               x (fudge ?? 1.0)
```
...
```
visualWidth = default_width x _width_scale x DEPTH_SCALE[depth] x scale
labelWidth  = layout.label_width
footprint   = max(visualWidth, min(labelWidth, visualWidth x MAX_FOOTPRINT_RATIO))
```
...
- `tab_stop_overflow` (warn) - total footprint of the left + center +
  right buckets + 2 x `zone_gap` exceeds zone width. Emitted from the
...
**Wrap rule.** If `label.length x AVG_CHAR_WIDTH_PCT > label_width x 1.1`,
...
Expected `final` (with viewport 1920x1080, `WORKSPACE_PX_PER_CM.bench = 3.2`):
...
```
_width_scale  = (display_width_cm x px_per_cm) / (default_width x PX_PER_SCENE_PERCENT)
              = (cm x 3.2) / (default_width x 11.52)

_visualWidth  = default_width x _width_scale x DEPTH_SCALE[depth]    (depth = mid -> 1.0)
_height       = _visualWidth x (1920 / 1080) / aspect
...
footprint = max(_visualWidth, min(label_width, _visualWidth x MAX_FOOTPRINT_RATIO))
```

Heat_block worked through: `(25 x 3.2) / (18 x 11.52) = 0.3858`, `_visualWidth =
18 x 0.3858 = 6.95`, `_height = 6.95 x 1.778 / 1.35 = 9.15`, `_top = 72 - 9.15 = 62.85`.
For tab-stops items, `padded_left = 5 + 1.5 = 6.5`, `padded_right = 95 - 1.5 = 93.5`;
microtube footprint = `max(3.33, min(10, 3.33 x 2.5)) = 8.33`, `_x = 6.5 + 4.165 = 10.66`.
