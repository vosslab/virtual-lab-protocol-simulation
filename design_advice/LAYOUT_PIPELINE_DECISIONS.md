...
`px_per_cm` per scene, and `width_scale = (display_width_cm x px_per_cm)
/ (default_width x 11.52)`. Earlier pipeline drafts treated `width_scale`
...
x (1080/1920)`. At non-16:9 viewports this changed heights non-trivially.
Intentional or bug?

**Decision.** **Bug.** Correct formula:

```
heightPct = visualWidth x (viewport.w / viewport.h) / aspect
```
...
viewport. Verifiable by porter: assert `visualWidth x viewport.w ===
visualHeight x viewport.h x aspect` (within FP tolerance).
...
+ 2 x zone_gap) > zone_width`. If so, emit warn with overflow magnitude.
...
`AVG_CHAR_WIDTH_PCT = 0.6` x label length; if > `label_width x 1.1`,
...
visualWidth x MAX_FOOTPRINT_RATIO))`), which is non-obvious. Microtube
...
10/10 scenes failing precheck at 1920x1080. The trials are evidence
...
| **Hardcoded viewport** | `.scene-container { width: 1920px; height: 1080px }`. A 6-viewport sweep found 1920x1080 was the least-bad - all 6 viewports failed. | Viewport is a `runPipeline` input. Aspect-adjust formula compensates. Invariant: "square asset renders square in pixels at every viewport." |
| **Placement-card crush** | `.footprint--handheld { max-height: 180px }` shrank container without shrinking SVG. p200 micropipette went 213 -> 180 px (16% squash), aspect 61.7% -> 79.7%. Every "make it fit" CSS fix that doesn't change the asset itself ends up violating PRIMARY_DESIGN. | Stage 8 derives height from `visualWidth x viewport_aspect / aspect`. Aspect is invariant by construction. Shrink only happens uniformly via `MIN_SCALE = 0.55`. |
...
| **YAML&harr;HTML divergence** | 1/10 scenes share canonical `zones[]` vocabulary with `content/base_scenes/*.yaml`. 8/10 use experiment-local taxonomy. | YAML is single source of truth. HTML is emitted. No way to drift. |
