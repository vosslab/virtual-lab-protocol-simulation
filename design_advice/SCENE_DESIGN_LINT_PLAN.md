...
| Metric | Definition | Targets |
|---|---|---|
| `zone_footprint_balance` | max(Σ footprint per zone) / min(Σ footprint per zone) over populated zones | composition ≤3×, dense ≤8× |
...
| Metric | Definition | Targets |
|---|---|---|
| `tab_stops_symmetry` | for zones using tab-stops, max/min of left/center/right footprint sums | ≤3×; **gated** on tab_stops_used > 0 |
