 ...
 * Downstream WPs extend this file: TimedWait (1C), RenderRequest (1D).
 */
...
export function applyObjectStateChange(
	...
	if (op.type !== 'ObjectStateChange') {
		throw new Error(`applyObjectStateChange requires type 'ObjectStateChange', got '${op.type}'`);
	}

	if (!op.target) {
		throw new Error(`missing required field 'target' on ObjectStateChange operation`);
	}

	if (!op.state) {
		throw new Error(`missing required field 'state' on ObjectStateChange operation for target '${op.target}'`);
	}
	...
	if (!(op.target in world.objectStates)) {
		throw new Error(`ObjectStateChange target '${op.target}' not found in objectStates`);
	}
...
export function applySceneChange(
	...
	if (op.type !== 'SceneChange') {
		throw new Error(`applySceneChange requires type 'SceneChange', got '${op.type}'`);
	}

	if (!op.to_scene) {
		throw new Error(`missing required field 'to_scene' on SceneChange operation`);
	}

	// Ensure the target scene exists; do not silently default.
	if (!(op.to_scene in world.scenes)) {
		throw new Error(`SceneChange target scene '${op.to_scene}' not found in scenes`);
	}
...
export function applyCursorAttach(
	...
	if (op.type !== 'CursorAttach') {
		throw new Error(`applyCursorAttach requires type 'CursorAttach', got '${op.type}'`);
	}

	if (!op.target) {
		throw new Error(`missing required field 'target' on CursorAttach operation`);
	}

	if (!op.operation) {
		throw new Error(`missing required field 'operation' on CursorAttach operation for target '${op.target}'`);
	}

	// Ensure the target exists in objectStates; do not silently default.
	if (!(op.target in world.objectStates)) {
		throw new Error(`CursorAttach target '${op.target}' not found in objectStates`);
	}

	// Validate operation is one of the allowed values.
	const validOperations = ['attach', 'detach'];
	if (!validOperations.includes(op.operation as string)) {
		throw new Error(`CursorAttach operation must be 'attach' or 'detach', got '${op.operation}'`);
	}
...
export function applyLayoutMove(
	...
	if (op.type !== 'LayoutMove') {
		throw new Error(`applyLayoutMove requires type 'LayoutMove', got '${op.type}'`);
	}
	...
	if (!op.operation) {
		throw new Error(`missing required field 'operation' or position info on LayoutMove operation for target '${op.target}'`);
	}
...
export function applyTimedWait(
	...
	if (op.type !== 'TimedWait') {
		throw new Error(`applyTimedWait requires type 'TimedWait', got '${op.type}'`);
	}
	...
	if (op.duration_min === null || op.duration_min === undefined) {
		throw new Error(`missing required field 'duration_min' on TimedWait operation for target '${op.target}'`);
	}
...
export function applySceneOperation(
	...
	switch (opType) {
		...
		case 'RenderRequest':
			throw new Error(`applySceneOperation: ${opType} is not yet implemented (deferred to later WP)`);
		default:
			throw new Error(`applySceneOperation: unknown operation type '${opType}'`);
	}
}
