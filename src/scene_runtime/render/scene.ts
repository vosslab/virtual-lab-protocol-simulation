...
export function renderScene(
  ...
  if (!(sceneId in world.scenes)) {
    throw new Error(
      `renderScene: scene '${sceneId}' not found in world.scenes`,
    );
  }
  ...
  const layoutMap = new Map(
    computedLayout.map((layout) => [layout.id, layout]),
  );

  // Create the scene root element (SVG for geometry and placement).
  const sceneRoot = document.createElementNS(
...
function resolveVisualAsset(
  ...
  for (const [fieldName, visualStateEntry] of Object.entries(
    objectConfig.visual_states,
  )) {
    // Skip deferred mechanisms.
    if (
      visualStateEntry.kind === "composite" ||
      visualStateEntry.kind === "overlay" ||
      visualStateEntry.kind === "formula"
    ) {
      continue;
    }
    ...
    if (
      visualStateEntry.kind !== "svg_swap" &&
      visualStateEntry.kind !== "svg"
    ) {
      continue;
    }
    ...
    if (visualStateEntry.cases) {
      for (const caseEntry of visualStateEntry.cases) {
        if (caseEntry.when === currentValue) {
          ...
          const assetName =
            caseEntry.output?.asset_name || caseEntry.asset_name;
          if (assetName) {
...
function renderPlacement(
  ...
  if (!(placement.object_name in world.objects)) {
    throw new Error(
      `renderPlacement: object '${placement.object_name}' not found in world.objects`,
    );
  }
  const objectConfigOrUndefined = world.objects[placement.object_name];
  if (!objectConfigOrUndefined) {
    throw new Error(
      `renderPlacement: object '${placement.object_name}' resolved to undefined`,
    );
  }
  ...
  if (!layout) {
    throw new Error(
      `renderPlacement: no computed layout for placement '${placement.placement_name}'`,
    );
  }
  ...
  const lines =
    layout.labelLines && layout.labelLines.length > 0
      ? layout.labelLines
