...
export function compute_scene_layout_css_native(
  ...
  if (typeof document === "undefined") {
    throw new Error(
      "compute_scene_layout_css_native: requires a DOM; not callable in non-browser environments",
    );
  }
  ...
  if (!scene) {
    throw new Error(
      `compute_scene_layout_css_native: scene '${scene_name}' not found in world.scenes`,
    );
  }
