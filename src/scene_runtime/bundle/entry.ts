 ...
 *
 * This file handles dynamic validator data from the builder (Record<string, any>).
 * Disabled unsafe-access rules for accessing properties on dynamic data structures.
 */
 ...
 */
function getExpectedValueFromValidator(
  validator: ValidatorPreset | Record<string, any>,
): Record<string, number | string | boolean> | null {
  if (
    typeof validator === "object" &&
    validator &&
    "preset" in validator &&
    validator.preset === "target_with_value" &&
    "value" in validator
  ) {
 ...
 */
function getFieldNameFromValidator(
  validator: ValidatorPreset | Record<string, any>,
): string | null {
...
function advanceStep(
  ...
  if (currentStep.next_step === null) {
    ...
    renderFeedback(
      feedbackArea,
      "Protocol complete! You have successfully completed all steps.",
    );
    return;
  }

  // Find the next step by name
  const nextStep = world.protocol.steps.find(
    (s) => s.step_name === currentStep.next_step,
  );
  if (!nextStep) {
    throw new Error(
      `Next step "${currentStep.next_step}" not found in protocol.steps`,
    );
  }
  ...
  if (nextStepIndex < 0) {
    throw new Error(
      `Next step "${currentStep.next_step}" not found in protocol.steps array`,
    );
  }
...
function orchestrateNextInteraction(
  ...
  if (interaction.gesture === "adjust") {
    ...
    if (!objectName) {
      throw new Error(
        `invalid target "${interaction.target}": no object name found`,
      );
    }
    ...
    if (!obj) {
      throw new Error(
        `Object "${objectName}" referenced by adjust interaction not found in world.objects`,
      );
    }

    // Find the state field matching fieldName.
    const stateField = obj.state_fields.find(
    ...
    if (!currentState) {
      throw new Error(
        `Object "${objectName}" has no state entry in world.objectStates`,
      );
    }
    ...
    renderAdjustPanel(adjustPanelContainer, {
      ...
      onCommit: (value: number) => {
        ...
        for (const sceneOp of interaction.response.scene_operations) {
          updatedWorld = applySceneOperation(
            updatedWorld,
            sceneOp,
            simpleClock,
          );
        ...
        const sceneViewport = document.querySelector(
    ...
    for (const groupCategory of Object.values(structure.subpart_groups)) {
      ...
      if (groupCat.members) {
        for (const member of groupCat.members) {
          if (
            member.name === expectedGroup &&
            member.contains &&
            member.contains.includes(emittedSubpart)
          ) {
            return true;
          }
        }
...
export async function loadAndMountByProtocolName(
  ...
  try {
    ...
    const { PROTOCOL_CONFIGS } =
      await import("../../../generated/protocol_data");
    ...
    if (!protocolName || typeof protocolName !== "string") {
      throw new Error(
        "loadAndMountByProtocolName: protocolName must be a non-empty string",
      );
    }
    ...
    const world = loadWorld(protocolName as any, {
      loadProtocol: (name: any) => {
        if (!(name in PROTOCOL_CONFIGS)) {
          throw new Error(`protocol "${name}" not found in PROTOCOL_CONFIGS`);
        }
        const protocol = PROTOCOL_CONFIGS[name];
        if (!protocol) {
          throw new Error(
            `protocol "${name}" is null or undefined in PROTOCOL_CONFIGS`,
          );
        }
        return protocol;
      },
      loadScene: (name: any) => {
        ...
        if (!scene) {
          throw new Error(
            `scene "${name}" is null or undefined in SCENE_CATALOG`,
          );
        }
        return scene;
      },
      loadObject: (name: any) => {
        ...
        if (!obj) {
          throw new Error(
            `object "${name}" is null or undefined in OBJECT_CATALOG`,
          );
        }
        return obj;
      },
      loadMaterial: (protocol: any, name: string) => {
        if (!protocol.materials || !(name in protocol.materials)) {
          throw new Error(
            `material "${name}" not found in protocol "${protocol.protocol_name}"`,
          );
        }
        const material = protocol.materials[name];
        if (!material) {
          throw new Error(
            `material "${name}" is null or undefined in protocol "${protocol.protocol_name}"`,
          );
        }
        return material;
      },
      scenesContainingObject: (objectName: string) => {
        ...
        for (const sceneName of Object.keys(SCENE_CATALOG)) {
          ...
          if (scene && Array.isArray(scene.placements)) {
            if (
              scene.placements.some((p: any) => p.object_name === objectName)
            ) {
    ...
    const { sceneViewport, promptPanel, feedbackArea, nextButton } =
      mountSceneFrame(rootElement);
