...
import type { PlacementConfig, ObjectConfig, SubpartGroupConfig, RuntimeWorld } from '../../types';
...
export function renderWellPlate(
	...
	const subpartGroups = objectConfig.structure?.subpart_groups as Record<string, SubpartGroupConfig> || {};

	// Pre-create group containers for each declared subpart group.
	for (const [_groupCategoryKey, groupCategory] of Object.entries(subpartGroups)) {
	...
	for (const [groupName, groupContainer] of groupContainers) {
		...
		if (isFinite(minX) && isFinite(minY) && isFinite(maxX) && isFinite(maxY)) {
			const bboxRect = groupContainer.querySelector('rect[pointer-events="none"]') as SVGElement;
			if (bboxRect) {
