...
test("Scene and Object loaders", async (suite) => {
	...
	const sceneLoader = await importTsModule(
	...
	await suite.test("Load a real scene and validate placements", () => {
		...
		assert.ok(
			loadedScene.workspace,
			"Loaded scene has workspace"
		);
		assert.ok(
		...
		for (const placement of loadedScene.placements) {
			assert.ok(
				...
				`Placement must have placement_name`
			);
			assert.ok(
				placement.object_name,
				`Placement must have object_name`
			);

			// Load the referenced object to prove it exists.
			const brandedObjectId = objectLoader.ObjectId(
				placement.object_name
			);
	...
	await suite.test("Load a real object and validate state_fields defaults", () => {
		...
		for (const stateField of loadedObject.state_fields) {
			assert.ok(
				stateField.field_name,
				`State field must have field_name`
			);
			assert.ok(
				stateField.type,
				`State field must have type`
			);
	...
	await suite.test("Negative test: reject unknown object reference in scene", () => {
	...
	await suite.test("Negative test: reject unresolved inheritance keys in scene", () => {
		...
		sceneData.SCENE_CATALOG["fake_scene_unresolved"] = fakeSceneWithInheritance;
