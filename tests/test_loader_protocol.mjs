...
const { PROTOCOL_CONFIGS } = await import("../generated/protocol_data.ts");
...
const protocolNames = Object.keys(PROTOCOL_CONFIGS);

test("loadProtocol loads at least one real protocol", (_t) => {
...
test("loadProtocol returns valid protocol shape", (_t) => {
  // Pick a mini_protocol (not a sequence_runner which has empty steps).
  const miniProtocols = protocolNames.filter(
    (name) => PROTOCOL_CONFIGS[name].protocol_type === "mini_protocol",
  );

  assert(
    miniProtocols.length > 0,
    "no mini_protocol found in PROTOCOL_CONFIGS",
  );

  const protocolName = miniProtocols[0];
  const proto = PROTOCOL_CONFIGS[protocolName];
...
test("ProtocolName constructor validates protocol existence", (_t) => {
...
test("loadProtocol validates all steps have required fields", (_t) => {
  ...
  for (const step of loaded.steps) {
    ...
    assert(
      typeof step.step_validator === "object",
      "step_validator must be object",
    );
  ...
  console.log(`  ✓ All steps in "${protocolName}" have required fields`);
});
