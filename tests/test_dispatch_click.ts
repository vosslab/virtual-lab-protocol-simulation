/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-floating-promises, @typescript-eslint/no-this-alias */
/**
...
function simulateClick(
  root: MockHTMLElement,
  targetElement: MockHTMLElement,
): void {
  const listeners = root._getListeners();
  const clickListeners = listeners.filter(
    (l) => l.type === "click" && l.useCapture,
  );
...
test("(a) click on target with data-target-id emits InteractionEvent", () => {
  ...
  const target = createMockElement({
    "data-target-id": "flask_1",
    "data-gesture": "click",
  });
  root._children = [target];
  target._parentElement = root;

  const detach = attachClickDispatch(root, (e) => events.push(e));
