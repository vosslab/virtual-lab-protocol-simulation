...
class StubElement {
  ...
  querySelector(selector) {
    ...
    for (const child of this.children) {
      if (
        selector === "[data-liquid-fill]" &&
        child.getAttribute("data-liquid-fill")
      ) {
        return child;
      }
  ...
  querySelectorAll(selector) {
    ...
    const walk = (el) => {
      ...
      if (
        selector === "[data-liquid-fill]" &&
        el.getAttribute &&
        el.getAttribute("data-liquid-fill")
      ) {
        results.push(el);
      }
      if (
        selector.startsWith(".") &&
        el.classList &&
        el.classList.has(selector.slice(1))
      ) {
        results.push(el);
      }
      // Walk children
...
test("renderLiquid: DMSO material (Pilot 1)", (_t) => {
  const dmsoMaterial = {
    label: "DMSO solvent",
    display_color: {
      light: "#007576",
...
test("renderLiquid: Formazan DMSO solution (Pilot 1)", (_t) => {
...
test("renderLiquid: default fillLevel is 1.0 (full)", (_t) => {
  const material = {
    label: "Test",
    display_color: {
      light: "#000000",
  ...
  assert.equal(
    fillRect.getAttribute("height"),
    "100",
    "Full height when no fillLevel",
  );
});

test("renderLiquid: _theme light vs dark", (_t) => {
  const material = {
    label: "Test",
    display_color: {
      light: "#aaaaaa",
  ...
  // Light theme
  const lightContainer = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "svg",
  );
  ...
  // Dark theme
  const darkContainer = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "svg",
  );
...
test("renderLiquid: _throws on missing display_color", (_t) => {
  const badMaterial = {
...
test("renderLiquid: _throws on missing light/dark in display_color", (_t) => {
  const badMaterial = {
...
test("renderLiquid: _throws on invalid hex color", (_t) => {
  const badMaterial = {
...
test("renderLiquid: _throws on null material", (_t) => {
  const container = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "svg",
  );
...
test("renderHighlight: adds highlight class _to matching element", (_t) => {
...
test("renderHighlight: clears previous highlight before adding new one", (_t) => {
  const container = document.createElement("div");
...
test("renderHighlight: null _targetId clears all highlights", (_t) => {
  const container = document.createElement("div");
  const target = document.createElement("div");
...
test("renderHighlight: handles missing _targets gracefully", (_t) => {
  const container = document.createElement("div");
