...
test("Runtime does not advance on wrong _target", async (_t) => {
  ...
  assert.strictEqual(
    validatorPassed,
    false,
    "Validator should reject wrong target",
  );
  ...
  assert.strictEqual(
    validatorPassed2,
    true,
    "Validator should pass for correct target",
  );
  ...
  assert.strictEqual(
    ...
    true,
    "State should be mutated after validator pass",
  );
});

test("Runtime does not mutate state before validator passes", async (_t) => {
  const world = createMockWorld();
