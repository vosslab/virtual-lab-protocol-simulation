...
const rendererModule = await importTsModule('src/scene_runtime/render/apply.ts');
...
function createTestWorld() {
	return {
		...
		scenes: {
			...
			incubator_workspace: { scene_name: 'incubator_workspace', placements: [] },
		},
		objects: {
			serological_pipette: { object_name: 'serological_pipette', kind: 'pipette' },
...
test('CursorAttach: attach operation updates cursorState', () => {
	...
	const originalCursorState = world.cursorState;
...
test('CursorAttach: detach operation clears cursorState', () => {
	...
	world.cursorState = { attachedTo: 'serological_pipette', operation: 'attach' };
...
test('LayoutMove: updates layout state for target', () => {
	...
	assert.strictEqual(nextWorld.layoutState.treatment_plate.target, 'treatment_plate');
	assert.strictEqual(nextWorld.layoutState.treatment_plate.operation, 'reposition');
...
test('LayoutMove: preserves existing layout state for other targets', () => {
	...
	assert.strictEqual(nextWorld.layoutState.treatment_plate.target, 'treatment_plate');
	assert.strictEqual(nextWorld.layoutState.treatment_plate.operation, 'new_move');
...
test('CursorAttach and LayoutMove: can be applied in sequence', () => {
	...
	assert.strictEqual(afterLayout.layoutState.serological_pipette.operation, 'follow_cursor');
