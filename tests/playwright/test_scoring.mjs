...
/* global calculateScore, gameState */
...
async function runTests(page) {
  ...
  try {
    ...
    results.push({
      ...
      detail: pass
        ? "ok"
        : `Expected total >= 90 and stars === 3, got total=${result.totalPoints}, stars=${result.stars}`,
    });
  } catch (e) {
    results.push({
      name: "Happy path: fresh gameState, monotonic profile, score >= 90, stars === 3",
  ...
  try {
    ...
    results.push({
      ...
      detail: pass
        ? "ok"
        : `Expected total < 60 and stars <= 2, got total=${result.totalPoints}, stars=${result.stars}`,
    });
  } catch (e) {
    results.push({
      name: "Multiple errors path: score < 60, stars <= 2",
  ...
  try {
    ...
    const pass =
      result.hasDilutionAccuracy &&
      result.hasPlateMap &&
      result.hasTiming &&
      result.hasMttTechnique &&
      result.hasAbsorbancePlausibility;
    results.push({
  ...
  try {
    const result = await page.evaluate(() => {
      ...
      const monoAbsorbancePoints =
        monoScore.categories.absorbancePlausibility.points;
      ...
      const jumbledAbsorbancePoints =
        jumbledScore.categories.absorbancePlausibility.points;
    ...
    results.push({
      ...
      detail: pass
        ? "ok"
        : `Jumbled (${result.jumbledPoints}) should be < Monotonic (${result.monoPoints})`,
    });
  } catch (e) {
    results.push({
      name: "Non-monotonic absorbance reduces absorbancePlausibility score",
  ...
  try {
    ...
    const pass =
      !result.hasOrder && !result.hasCleanliness && !result.hasWastedMedia;
    results.push({
