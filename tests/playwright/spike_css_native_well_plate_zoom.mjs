...
async function main() {
	const stylesDir = path.join(REPO_ROOT, 'experiments', 'css_native_layout', 'styles');
	...
	record('lane3.flag_enabled', lane3.enabled === true,
		'is_enabled() returned ' + lane3.enabled);
	record('lane3.result_is_array_of_length_1', Array.isArray(lane3.layouts) && lane3.layouts.length === 1,
		'length=' + (lane3.layouts ? lane3.layouts.length : 'undefined'));

	const item = (lane3.layouts && lane3.layouts[0]) || {};
	record('lane3.width_positive', typeof item.width === 'number' && item.width > 0,
		'width=' + item.width);
	record('lane3.height_positive', typeof item.height === 'number' && item.height > 0,
		'height=' + item.height);
	record('lane3.x_is_number', typeof item.x === 'number', 'x=' + item.x);
	record('lane3.y_is_number', typeof item.y === 'number', 'y=' + item.y);
	record('lane3.id_matches_placement_name', item.id === 'zoom_well_plate_96',
		'id=' + item.id);

	await page.screenshot({ path: screenshotPath, fullPage: false });
	record('lane3.screenshot_saved', fs.existsSync(screenshotPath),
		'path=' + screenshotPath);
	...
	console.log('PERF: median_ms=' + medianMs
		+ ' p95_ms=' + p95Ms
		+ ' max_ms=' + maxMs
	...
	record('lane4.no_dom_leak', bodyDelta === 0,
		'body_children_delta=' + bodyDelta);
	...
	record('lane4.scaffold_node_count_sane', perf.nodes >= 2 && perf.nodes <= 6,
		'nodes_per_call=' + perf.nodes);

	// Perf sanity: median should be well under 50ms in a headless Chromium.
	record('lane4.median_under_50ms', perf.median < 50.0,
		'median_ms=' + medianMs);

	await browser.close();

	// Tidy the temporary host HTML now that the browser is closed.
	try { fs.unlinkSync(hostHtmlPath); } catch (e) { /* best effort */ }
