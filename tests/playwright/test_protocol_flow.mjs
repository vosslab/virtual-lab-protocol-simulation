...
/* global PROTOCOL_STEPS, computeWellViability, gameState, renderProtocolUI */
...
async function runTests(page) {
  ...
  try {
    const validation = await page.evaluate(() => {
      ...
      for (let i = 0; i < PROTOCOL_STEPS.length; i++) {
        ...
        if (!step.id || step.id.length === 0)
          issues.push(`Step ${i}: missing id`);
        if (!step.action || step.action.length === 0)
          issues.push(`Step ${i}: missing action`);
        if (step.action.length > 60)
          issues.push(
            `Step ${i}: action too long (${step.action.length} > 60)`,
          );
        if (!step.why || step.why.length === 0)
          issues.push(`Step ${i}: missing why`);
        if (step.why.length > 100)
          issues.push(`Step ${i}: why too long (${step.why.length} > 100)`);
        ...
        if (typeof step.stepIndex !== "number")
          issues.push(`Step ${i}: missing stepIndex`);
        if (!step.requiredItems || step.requiredItems.length === 0)
          issues.push(`Step ${i}: missing or empty requiredItems`);
      }
      return issues;
    });
  ...
  try {
    ...
    results.push({
      ...
      detail: pass
        ? "ok"
        : `Expected empty array, got ${JSON.stringify(intros)}`,
    });
  } catch (e) {
    results.push({
      name: "gameState.seenPartIntros is an empty array",
  ...
  try {
    ...
    results.push({
      ...
      pass,
      detail: pass
        ? `ok (${viability.toFixed(3)})`
        : `Expected ~0.55, got ${viability.toFixed(3)}`,
    });
  } catch (e) {
    results.push({
      name: "computeWellViability(5, false) is approximately 0.55",
  ...
  try {
    ...
    results.push({
      ...
      pass,
      detail: pass
        ? `ok (no-met: ${v1.toFixed(3)}, +met: ${v2.toFixed(3)})`
        : `Expected lower with metformin, got ${v2.toFixed(3)} >= ${v1.toFixed(3)}`,
    });
  } catch (e) {
    results.push({
      name: "computeWellViability(5, true) < computeWellViability(5, false)",
  ...
  try {
    ...
    const hasAll =
      html.includes("day-pill") &&
      html.includes("step-action") &&
      html.includes("step-why");
    results.push({
