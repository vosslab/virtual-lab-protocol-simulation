 ...
 *
 * Run: node tests/test_step_completeness.mjs
 */

/* global EQUIPMENT, PROTOCOL_STEPS */
...
// Helper: derive usedItems from interactionSequence in first-use order (tool -> source -> destination, de-duplicated)
function __deriveUsedItems(step) {
  const seen = new Set();
  const used = [];
...
async function runCompletenesTest(page) {
  ...
  const auditData = await page.evaluate(() => {
    // Helper to derive used items from interactionSequence
  ...
  // Test 1: Every usedItem in requiredItems or is virtual_target
  console.log(
    "Test 1: Used items must be in required items or be virtual targets",
  );
  for (const step of steps) {
    for (const target of step.usedItems) {
      ...
      if (!inRequired && !isVirtual) {
        results.push({
          pass: false,
          name: `${step.id}: usedItem '${target}' not in requiredItems and not virtual_target`,
          step: step.id,
        });
      } else {
        results.push({
  ...
  console.log("Test 2: Used items must be in correct scene");
  for (const step of steps) {
    for (const target of step.usedItems) {
      ...
      if (!item) {
        results.push({
          pass: false,
          name: `${step.id}: usedItem '${target}' not found in EQUIPMENT`,
          step: step.id,
        });
        continue;
      }
  ...
  console.log(
    "Test 5: aspirate_old_media has waste_container in required and used",
  );
  const aspirateStep = steps.find((s) => s.id === "aspirate_old_media");
  if (aspirateStep) {
    const hasInRequired =
      aspirateStep.requiredItems.includes("waste_container");
    const hasInUsed = aspirateStep.usedItems.includes("waste_container");
    results.push({
      pass: hasInRequired,
    ...
    results.push({
      pass: hasInUsed,
      name: `aspirate_old_media has waste_container in usedItems: ${hasInUsed}`,
    });
  }

  // Test 6: count_cells has both cell_counter and microscope as used
  console.log(
    "Test 6: count_cells has microscope as alternate target (M2 requirement)",
  );
  const countStep = steps.find((s) => s.id === "count_cells");
  if (countStep) {
    const hasCellCounter = countStep.usedItems.includes("cell_counter");
    const hasMicroscope = countStep.usedItems.includes("microscope");
    results.push({
      pass: hasCellCounter,
      name: `count_cells has cell_counter as used item: ${hasCellCounter}`,
    });
    results.push({
      pass: hasMicroscope,
      name: `count_cells has microscope as alternate used item: ${hasMicroscope}`,
    });
  }
