...
async function getVisibilityInfo(locator) {
...
//============================================
async function getDomSnippet(locator) {
  // Extract the outer HTML for error diagnostics.
  const snippet = await locator.evaluate((el) => {
 ...
 */
export async function resolveAndClick(page, target) {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    for (const selector of selectors) {
      ...
      if (visibility.isVisible) {
        console.log(
          `[click_resolver] Clicking target '${target.value}' (field '${target.yamlField}')`,
        );
