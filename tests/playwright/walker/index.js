...
import { captureBefore, captureAfter, captureStepSummary, ensureScreenshotDir, } from './screenshot.js';
...
async function executeStep(page, step, protocolId, stepIndex) {
    ...
    if (!completed.includes(step.id)) {
        ...
        throw new Error(`Step ${step.id}: not present in gameState.completedSteps after execution (saw: ${JSON.stringify(completed)})`);
    }
...
export async function walkProtocol(opts) {
    const { protocolId, baseUrl = 'http://localhost:8000', headless = true, screenshotDir = `test-results/walker/${protocolId}`, protocolConfig: optionalConfig, } = opts;
    let browser;
    let page;
    try {
