 ...
 */
async function advanceTestClock(page, seconds) {
 ...
 */
async function locateTarget(page, target, gesture) {
...
async function dispatchAdjust(page, interaction) {
	...
	if (!isVisible) {
		throw new Error(
			`Adjust input exists but not visible: ${inputSelector}`
		);
	}
...
async function executeStep(
	...
	const { step_name, prompt, sequence, next_step } = step;
	...
	for (let i = 0; i < sequence.length; i++) {
		...
		log(`  Interaction ${i + 1}/${sequence.length}: ${interaction.gesture} on ${interaction.target}`, verbosity, 'debug');
	...
	if (!nextVisible) {
		...
		const runtimeState = await page.evaluate(() => {
			...
			return {
				...
				stepLength: w.__RUNTIME_PROTOCOL_CONFIG?.world?.protocol?.steps?.[w.__RUNTIME_PROTOCOL_CONFIG?.world?.activeStepIndex || 0]?.sequence?.length,
				clickLog: (w.__RUNTIME_CLICK_LOG || []).slice(-10).map(c => `[${c.targetId}@idx${c.currentInteractionIndex}]`),
				checkStepLog: (w.__RUNTIME_CHECK_STEP_LOG || []).slice(-10).map(c => `[step${c.activeStepIndex}@idx${c.currentInteractionIndex}]`),
				orchestrateLog: (w.__RUNTIME_ORCHESTRATE_LOG || []).slice(-10).map(o => `[step${o.activeStepIndex}@idx${o.currentInteractionIndex}]`),
				orchestrateCallLog: (w.__RUNTIME_ORCHESTRATE_CALL_LOG || []).slice(-20).map(c => `[step${c.step}@idx${c.idx}]`),
				clickError: w.__RUNTIME_CLICK_ERROR,
				orchestrateDetail: (w.__RUNTIME_ORCHESTRATE_DETAIL_LOG || []).map(d => `[${d.event}@step${d.activeStepIndex}]`),
...
function loadProtocolFromGenerated(protocolName) {
	...
	tsContent = tsContent.replace(/import\s+type\s+{[^}]*}\s+from\s+['"][^'"]*['"]/g, '');
	// Replace type annotations on export
	tsContent = tsContent.replace(/export const PROTOCOL_CATALOG: Record<string, ProtocolConfig> = /, 'const PROTOCOL_CATALOG = ');
	...
	try {
		...
		if (!catalog[protocolName]) {
			throw new Error(`Protocol "${protocolName}" not found in PROTOCOL_CATALOG`);
		}

		return catalog[protocolName];
	} catch (err) {
		// If parsing/eval fails, return a minimal config with just the name
		log(`Warning: Could not parse full protocol config: ${err.message}`, 'info');
...
export async function runWalker(opts) {
	...
	const finalBaseUrl = baseUrl || `file://${path.resolve(process.cwd(), `dist/${protocolName}.html`)}`;
	...
	try {
		...
		try {
			...
			log(`Loaded protocol "${protocolName}" with ${protocol.steps?.length || 0} steps from generated data`, verbosity);
		} catch (err) {
		...
		log(`Loaded protocol "${protocol.protocol_name}" with ${protocol.steps.length} steps`, verbosity);

		// Verify entry step
		const entryStep = protocol.steps.find(s => s.step_name === protocol.entry_step);
		if (!entryStep) {
			throw new Error(
				`Entry step "${protocol.entry_step}" not found in steps`
			);
		}
