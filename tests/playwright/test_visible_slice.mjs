...
async function runTest() {
  ...
  try {
    ...
    console.log(
      `Page has scene-container: ${hasSceneContainer}, has <svg: ${hasSvg}`,
    );
    ...
    const targetLocator = page.locator(
      'svg > g[data-target-id="media_bottle"]',
    );
    const count = await targetLocator.count();

    if (count === 0) {
      throw new Error(
        'media_bottle element not found (looking for svg > g[data-target-id="media_bottle"])',
      );
    }
    ...
    const fillBefore = await targetLocator
      .first()
      .evaluate((el) => {
    ...
    const fillAfter = await targetLocator
      .first()
      .evaluate((el) => {
        const rect = el.querySelector("rect");
        return rect ? rect.getAttribute("fill") : null;
    ...
    if (fillBefore && fillAfter && fillBefore !== fillAfter) {
      console.log(
        `DOM changed: fill attribute changed (${fillBefore} -> ${fillAfter})`,
      );
      domChanged = true;
    }
    ...
    if (!domChanged) {
      throw new Error(
        "FAILURE: No visual DOM delta detected after state change. Fill attribute must change on visual_states.material_name svg_swap.",
      );
    }
