...
async function loadSpikeHarness() {
  const spike_html_path = path.join(
    REPO_ROOT,
    "dist",
    "_spike_well_plate_96_zoom_check.html",
  );
  ...
  try {
    ...
    await page.addInitScript(() => {
      ...
      Object.defineProperty(window, "SceneRuntime", {
        ...
        set(value) {
          ...
          if (
            ...
            window.__spike_flag_set_count =
              (window.__spike_flag_set_count || 0) + 1;
            console.log(
              "[spike] Flag setter called, count=" +
                window.__spike_flag_set_count,
            );
          }
        },
      ...
      const clickHandler = (e) => {
        ...
        const targetWithId = target?.closest
        ...
        console.log(
          "[click-capture] Event #" +
            ...
            ", closest_id=" +
            (targetWithId?.getAttribute("data-target-id") || "none"),
        );
      };
    ...
    const runtime_bundle_content = await import("fs")
      .then(
        (fs) =>
    ...
    const flag_set_count = await page.evaluate(
      () => window.__spike_flag_set_count || 0,
    );
    ...
    console.log(
      `[assert] CSS-native invocation count at mount: ${invocation_count_mount}`,
    );
    if (invocation_count_mount < 1) {
      throw new Error(
        `FAILED: invocation_count_mount ${invocation_count_mount} < 1`,
      );
    }
    ...
    if (
      !viewport_bbox ||
      viewport_bbox.width === 0 ||
      viewport_bbox.height === 0
    ) {
    ...
    const before_click_screenshot = path.join(
      screenshot_dir,
      "lane_r_before_click.png",
    );
    ...
    const body_children_before = await page.evaluate(
      () => document.body.children.length,
    );
    console.log(
      `[info] document.body.children.length BEFORE click: ${body_children_before}`,
    );
    ...
    console.log(
      `[info] CSS-native invocation count BEFORE click: ${invocation_count_before_click}`,
    );
    ...
    const e7_elem_info = await page.evaluate(() => {
      const elem = document.querySelector(
        '[data-target-id="well_plate_96.E7"]',
      );
    ...
    console.log(
      `Click at E7 center (${center_x.toFixed(1)}, ${center_y.toFixed(1)})`,
    );

    // Debug: what's at the click position and what's the DOM structure?
    const debug_info = await page.evaluate(
      ({ x, y }) => {
        ...
        return {
          ...
          element_pointer_events: elem
            ? window.getComputedStyle(elem).pointerEvents
            : "N/A",
          svg_exists: Boolean(svg),
          svg_pointer_events: svg
            ? window.getComputedStyle(svg).pointerEvents
            : null,
    ...
    console.log(
      `[info] Click listener registered: ${click_state.listener_registered}`,
    );
    console.log(
      `[info] Click events captured: ${click_state.click_events.length} (count=${click_state.click_count})`,
    );
    for (const evt of click_state.click_events) {
      console.log(
        `[click] tag=${evt.target_tag}, class=${evt.target_class}, closest_id=${evt.closest_target_id}`,
      );
    }
    ...
    const spike_flag_still_enabled = await page.evaluate(() => {
      if (
        window.SceneRuntime &&
        window.SceneRuntime.__spike &&
        window.SceneRuntime.__spike.is_css_native_well_plate_zoom_spike_enabled
      ) {
    ...
    console.log(
      `[info] CSS-native invocation count AFTER click: ${invocation_count_after_click}`,
    );

    // Compute delta.
    const invocation_count_delta =
      invocation_count_after_click - invocation_count_before_click;
    ...
    console.log(
      `PASS: invocation count increased by ${invocation_count_delta}`,
    );

    // Record: DOM children count AFTER click.
    const body_children_after = await page.evaluate(
      () => document.body.children.length,
    );
    console.log(
      `[info] document.body.children.length AFTER click: ${body_children_after}`,
    );
    ...
    const after_click_screenshot = path.join(
      screenshot_dir,
      "lane_r_rerender.png",
    );
    ...
    console.log(
      `CSS-native invocation BEFORE click: ${invocation_count_before_click}`,
    );
    console.log(
      `CSS-native invocation AFTER click:  ${invocation_count_after_click}`,
    );
    console.log(
      `CSS-native invocation delta: ${invocation_count_delta} (PASS if > 0)`,
    );
    console.log(`DOM children before:         ${body_children_before}`);
    console.log(`DOM children after:          ${body_children_after}`);
    console.log(
      `DOM children delta:          ${body_children_delta} (PASS if == 0)`,
    );
