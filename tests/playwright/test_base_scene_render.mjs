...
`;
	...
	boundingBoxResult.forEach(box => {
		console.log(`    ${box.targetId}: (${box.width}x${box.height}) at (${box.x}, ${box.y})`);
	});
	...
	if (boundingBoxResult.length === sceneData.placements.length) {
		assertions.push(`PASS: All ${sceneData.placements.length} placements have data-target-id`);
	} else {
		assertions.push(`FAIL: Expected ${sceneData.placements.length} placements, found ${boundingBoxResult.length}`);
	}

	// Assertion 2: Every placed object has nonzero bounding box.
	const zeroBoxes = boundingBoxResult.filter(box => box.width === 0 || box.height === 0);
	...
	const positions = boundingBoxResult.map(box => `${Math.round(box.x)},${Math.round(box.y)}`);
	...
	if (uniqueSigs.size === boundingBoxResult.length) {
		...
		assertions.push(`WARN: Some objects have identical dimensions (may be intentional)`);
	}
...
// Run the test.
runTest().then(code => process.exit(code)).catch(err => {
