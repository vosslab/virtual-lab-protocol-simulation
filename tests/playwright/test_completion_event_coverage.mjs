...
async function runTests() {
  ...
  try {
    ...
    try {
      ...
      if (result === "strict") {
        console.log(
          '[PASS] Test 1: getCoveragePolicy("cell_culture") returns "strict"',
        );
        passCount++;
      } else {
    ...
    try {
      ...
      if (result === "relaxed") {
        console.log(
          '[PASS] Test 2: getCoveragePolicy("tutorial_split") returns "relaxed"',
        );
        passCount++;
      } else {
    ...
    try {
      ...
      if (result === "relaxed") {
        console.log(
          '[PASS] Test 3: getCoveragePolicy("tutorial_pbs") returns "relaxed"',
        );
        passCount++;
      } else {
    ...
    try {
      ...
      if (result === "strict") {
        console.log(
          '[PASS] Test 4: getCoveragePolicy("some_unknown_protocol") returns "strict"',
        );
        passCount++;
      } else {
        console.log(
          '[FAIL] Test 4: getCoveragePolicy("some_unknown_protocol")',
        );
        console.log("  Result:", result);
      }
    } catch (err) {
    ...
    try {
      const result = await page.evaluate(() => {
        ...
        if (missing.length > 0) {
          if (policy === "strict") {
            throw new Error(
              `dead step (no trigger wired): No scene calls triggerStep() for: ${missing.join(", ")}`,
            );
          }
        }
        ...
        return {
          errorThrown: false,
          protocolValidation: window.__protocolValidation,
        };
      });

      if (result.protocolValidation.ok === true) {
        console.log(
          "[PASS] Test 5: cell_culture with all emitters: no error, ok=true",
        );
        passCount++;
      } else {
    ...
    try {
      ...
      if (result.errorThrown && result.errorMessage.includes("step2")) {
        console.log(
          "[PASS] Test 6: cell_culture with missing emitter: throws error mentioning step2",
        );
        passCount++;
      } else {
    ...
    try {
        ...
        try {
          ...
          if (missing.length > 0) {
            if (policy === "strict") {
              ...
              console.warn(
                `[completion-event coverage] protocol '${PROTOCOL_ID}' missing emitters: ${missing.join(", ")}`,
              );
            }
          }
      ...
      if (
        !result.errorThrown &&
        result.warningLogged &&
        result.warningMessage.includes("tutorial_split") &&
        result.warningMessage.includes("step2") &&
        result.protocolValidation.ok === true
      ) {
        console.log(
          "[PASS] Test 7: tutorial_split with missing emitter: warns but does not throw, ok=true",
        );
        passCount++;
      } else {
    ...
    if (passCount === totalTests) {
      ...
      console.log(
        `completion_event_coverage: FAILED ${passCount}/${totalTests}`,
      );
      await browser.close();
      server.kill();
      process.exit(1);
