...
const STRESS_RESULTS_DIR = path.join(REPO_ROOT, 'experiments', 'css_native_layout', 'stress_results');
const ASSETS_DIR = path.join(REPO_ROOT, 'docs', 'active_plans', 'new3_layout_stress_reliability_assets');
const LOG_PATH = path.join(STRESS_RESULTS_DIR, 'batch2_interaction_log.txt');
const RESULTS_MD_PATH = path.join(STRESS_RESULTS_DIR, 'batch2_interaction_results.md');
...
async function mountSpikeHarness(page, capturedLogs) {
	...
	page.on('pageerror', (err) => {
		capturedLogs.push({ type: 'pageerror', text: String(err && err.message ? err.message : err) });
	});
	...
	const harness_html = readFileSyncUtf8(path.join(REPO_ROOT, 'dist', '_spike_well_plate_96_zoom_check.html'));
	await page.setContent(harness_html);

	const bundle = readFileSyncUtf8(path.join(REPO_ROOT, 'dist', 'runtime.bundle.js'));
	...
	await page.evaluate(() => {
		...
		if (window.SceneRuntime && window.SceneRuntime.__spike) {
			window.SceneRuntime.__spike.set_css_native_well_plate_zoom_spike_enabled_for_test(true);
		}
		window.SceneRuntime.loadAndMountByProtocolName(runtimeRoot, runtimeData.protocol_name);
	});
...
async function interaction_repeated_correct_clicks(page) {
	...
	logLine(`[i1] before: invocation=${before_invocation}, body.children=${before_children}, dom.nodes=${before_nodes}, mem=${before_mem}`);

	const center = await getTargetCenter(page, target_id);
	if (!center) throw new Error(`Target ${target_id} not found`);
	logLine(`[i1] target center: (${center.x.toFixed(1)}, ${center.y.toFixed(1)})`);

	// Screenshot BEFORE
	const before_shot = path.join(ASSETS_DIR, 'lane_i_repeated_correct_clicks_before.png');
	...
	logLine(`[i1] after:  invocation=${after_invocation}, body.children=${after_children}, dom.nodes=${after_nodes}, mem=${after_mem}`);
	...
	const mem_delta = (before_mem !== null && after_mem !== null) ? (after_mem - before_mem) : null;
	logLine(`[i1] delta:  invocation=${inv_delta}, body.children=${body_delta}, dom.nodes=${node_delta}, mem=${mem_delta}`);
	logLine(`[i1] mid-loop invocation after click #1: ${mid_invocation}`);

	// Screenshot AFTER
	const after_shot = path.join(ASSETS_DIR, 'lane_i_repeated_correct_clicks_after.png');
	...
	const pass_invocation = (mid_invocation !== null && mid_invocation > before_invocation);
...
async function interaction_wrong_target(page) {
	...
	const before_children = await getBodyChildren(page);
	const before_nodes = await getDomNodeCount(page);
	const before_console_len = (page.__captured_console_len || 0);
	...
	logLine(`[i2] target center: (${center.x.toFixed(1)}, ${center.y.toFixed(1)})`);
	...
	const body_delta = after_children - before_children;
	const node_delta = after_nodes - before_nodes;
	logLine(`[i2] after: invocation_delta=${inv_delta}, body.children_delta=${body_delta}, dom.nodes_delta=${node_delta}`);
	...
	recordResult({
		...
		screenshots: [before_shot, after_shot],
		pass,
		notes: 'Wrong target: protocol target is E7, A1 should not advance the step.',
	});
}
...
async function interaction_selected_state_visual(page) {
	logLine('');
	logLine('=== Interaction 3: selected well state visual (SKIP unless exposed) ===');
	...
	recordResult({
		...
		screenshots: [],
		pass: true,
		notes: 'SKIP - dev_smoke protocol does not expose selected state, and __spike API exposes no scene mutation entry point.',
	});
}
...
async function interaction_cross_object(page) {
	logLine('');
	logLine('=== Interaction 4: cross-object clicks (SKIP - single dev_smoke scene) ===');
	recordResult({
		...
		screenshots: [],
		pass: true,
		notes: 'SKIP - dist/_spike_well_plate_96_zoom_check.html harness only wires well_plate_96_zoom_check; no other dev_smoke scene available through this bundle.',
	});
}
...
async function interaction_state_change_cycles(page) {
	logLine('');
	logLine('=== Interaction 5: 50 dispatch cycles on E7 (post-completion idempotency) ===');
	const target_id = 'well_plate_96.E7';
	const center = await getTargetCenter(page, target_id);
	if (!center) throw new Error(`Target ${target_id} not found`);
	...
	logLine(`[i5] delta: invocation=${inv_delta}, body.children=${body_delta}, dom.nodes=${node_delta}, mem=${mem_delta}`);
...
async function interaction_race_condition(page) {
	...
	const body_delta = after_children - before_children;
	const node_delta = after_nodes - before_nodes;
	logLine(`[i6] delta: invocation=${inv_delta}, body.children=${body_delta}, dom.nodes=${node_delta}`);
...
function writeResultsMarkdown(capturedLogs, totals) {
	...
	lines.push('`tests/playwright/spike_batch2_interaction_stress.mjs` against the prebuilt');
	lines.push('`dist/runtime.bundle.js` and `dist/_spike_well_plate_96_zoom_check.html`.');
	lines.push('');
	lines.push('## Per-interaction results');
	lines.push('');
	lines.push('| Interaction | Iterations | Expected invocation delta (min) | Actual invocation delta | body.children delta | dom.nodes delta | mem delta (bytes) | PASS/FAIL |');
	lines.push('| --- | --- | --- | --- | --- | --- | --- | --- |');
	for (const r of results) {
		lines.push(`| ${r.name} | ${r.iterations} | ${r.expected_invocation_delta_min} | ${r.actual_invocation_delta} | ${r.body_children_delta} | ${r.dom_nodes_delta} | ${r.mem_delta_bytes === null ? 'n/a' : r.mem_delta_bytes} | ${r.pass ? 'PASS' : 'FAIL'} |`);
	}
	lines.push('');
	...
	lines.push('');
	lines.push(`- Total invocation count over all interactions: ${totals.total_invocation_observed}`);
	lines.push(`- Total body.children growth across all interactions: ${totals.total_body_children_growth}`);
	lines.push(`- Total dom.nodes growth across all interactions: ${totals.total_dom_nodes_growth}`);
	lines.push(`- Console errors / page errors captured: ${totals.console_errors}`);
	lines.push(`- Race condition observed: ${totals.race_observed ? 'YES' : 'NO'}`);
	lines.push(`- Memory growth observed (performance.memory): ${totals.memory_available ? `${totals.total_mem_growth_bytes} bytes` : 'n/a (performance.memory unavailable in Playwright Chromium build)'}`);
	lines.push('');
	lines.push('### Verdicts');
	lines.push('');
	lines.push(`- **DOM leak check (per-interaction, body.children delta == 0)**: ${totals.dom_leak_pass ? 'PASS' : 'FAIL'}.`);
	lines.push(`- **Race condition safety**: ${totals.race_pass ? 'PASS' : 'FAIL'} (no DOM leak, dispatch serialized via async handler).`);
	lines.push(`- **Wrong-target behavior**: ${totals.wrong_target_pass ? 'PASS' : 'FAIL'} (DOM stable; protocol target unchanged).`);
	lines.push(`- **Memory leak via performance.memory**: ${totals.memory_available ? (totals.total_mem_growth_bytes < 5_000_000 ? 'PASS (growth under 5 MB across all stress interactions)' : 'INVESTIGATE (growth >= 5 MB)') : 'NOT MEASURED'}.`);
	lines.push('');
...
async function main() {
	...
	try {
		...
		logLine(`[baseline] invocation=${baseline_invocation}, body.children=${baseline_children}, dom.nodes=${baseline_nodes}, mem=${baseline_mem}`);
		...
		const console_errors = capturedLogs.filter((c) => c.type === 'error' || c.type === 'pageerror').length;
		const dom_leak_pass = results.every((r) => r.body_children_delta === 0);
		const race_result = results.find((r) => r.name === 'race_condition_5x_fast');
		const race_pass = race_result ? race_result.pass : false;
		const wrong_target_result = results.find((r) => r.name === 'wrong_target_A1');
		const wrong_target_pass = wrong_target_result ? wrong_target_result.pass : false;
		const race_observed = race_result ? (race_result.body_children_delta !== 0) : false;
		...
		try { fs.writeFileSync(LOG_PATH, log_lines.join('\n') + '\n', 'utf8'); } catch (_e) { /* ignore */ }
	}
}
