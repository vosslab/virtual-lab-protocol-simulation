...
async function main() {
  ...
  try {
    ...
    await page.addInitScript(() => {
      ...
      Object.defineProperty(window, "__RUNTIME_PROTOCOL_CONFIG", {
        set(value) {
          ...
          console.log(
            "[protocol] Config set: " + value?.protocol?.protocol_name,
          );
        },
      });
    ...
    if (target_count < 1) {
      throw new Error(
        `FAILED: well_plate_96 target element count ${target_count} < 1`,
      );
    }
    ...
    console.log(
      `[info] well_plate_96 SVG bbox BEFORE click: ${JSON.stringify(sample_rect_before)}`,
    );
    ...
    console.log(
      `Click at viewport center (${center_x.toFixed(1)}, ${center_y.toFixed(1)})`,
    );
    ...
    while (Date.now() - step_complete_start < step_complete_timeout_ms) {
      step_complete_detected = await page.evaluate(
        () => window.__step_complete || false,
      );
    ...
    console.log(
      `[info] World state after click: ${JSON.stringify(world_state_after_click)}`,
    );
    ...
    console.log(
      `[info] well_plate_96 SVG bbox AFTER click: ${JSON.stringify(sample_rect_after)}`,
    );
    ...
    if (sample_rect_after) {
      ...
      console.log(
        "PASS: layout rect bounds valid (non-negative, reasonable size)",
      );
    }
    ...
    console.log(
      `CSS-native delta:            ${invocation_count_delta} (PASS if > 0)`,
    );
    ...
    console.log(
      `Sample rect before:          ${JSON.stringify(sample_rect_before)}`,
    );
    console.log(
      `Sample rect after:           ${JSON.stringify(sample_rect_after)}`,
    );
