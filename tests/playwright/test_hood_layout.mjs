  ...
  try {
    ...
    for (const viewport of TEST_VIEWPORTS) {
      ...
      const labelOverlapResult = await page.evaluate(() => {
        const labels = Array.from(
          document.querySelectorAll(".hood-item-label"),
        );
        ...
        for (let i = 0; i < boxes.length; i++) {
          for (let j = i + 1; j < boxes.length; j++) {
            ...
            if (aRight > bLeft && a.left < b.right) {
              overlaps.push(
                `"${a.text}" overlaps with "${b.text}" (a.right=${a.right.toFixed(1)} b.left=${b.left.toFixed(1)})`,
              );
            }
          }
        }
      ...
      const containmentResult = await page.evaluate(() => {
        const scene = document.getElementById("hood-scene");
        ...
        for (let i = 0; i < items.length; i++) {
          const el = items[i];
          const id = el.getAttribute("data-item-id") || "unknown";
          ...
          if (box.right > sceneBox.right + tol) {
            violations.push(
              id +
                " right edge outside scene (" +
                box.right.toFixed(0) +
                " > " +
                sceneBox.right.toFixed(0) +
                ")",
            );
          }
          if (box.top < sceneBox.top - tol) {
            violations.push(
              id +
                " top edge outside scene (" +
                box.top.toFixed(0) +
                " < " +
                sceneBox.top.toFixed(0) +
                ")",
            );
          }
          if (box.bottom > sceneBox.bottom + tol) {
            violations.push(
              id +
                " bottom edge outside scene (" +
                box.bottom.toFixed(0) +
                " > " +
                sceneBox.bottom.toFixed(0) +
                ")",
            );
          }
        }
        ...
        const labels = document.querySelectorAll(".hood-item-label");
      ...
      const itemOverlapResult = await page.evaluate(() => {
        const items = Array.from(document.querySelectorAll(".hood-item"));
        const boxes = items.map(function (el) {
          const r = el.getBoundingClientRect();
          return {
        ...
        for (let i = 0; i < boxes.length; i++) {
          for (let j = i + 1; j < boxes.length; j++) {
            ...
            const tol = 2;
            const hOverlap = a.right - tol > b.left && b.right - tol > a.left;
            const vOverlap = a.bottom - tol > b.top && b.bottom - tol > a.top;
    ...
    const flaskSizeResult = await page.evaluate(() => {
      ...
      const pipettes = Array.from(
        document.querySelectorAll(".hood-item"),
      ).filter((el) => {
      ...
      const narrowerThan = pipetteWidths.filter(
        (p) => flaskBox.width <= p.width,
      );
    ...
    printCheck(
      2,
      "Flask is visually prominent (wider than pipettes)",
      check2Pass,
      flaskSizeResult.detail,
    );
    ...
    const movedItems = await page.evaluate(() => {
      return {
        microscope:
          document.querySelector('#hood-scene [data-item-id="microscope"]') !==
          null,
        incubator:
          document.querySelector('#hood-scene [data-item-id="incubator"]') !==
          null,
      };
    });
    ...
    printCheck(
      "3b",
      "Microscope and incubator not present on hood",
      check3bPass,
    ...
    const layerResult = await page.evaluate(() => {
      ...
      return {
        hasItemsLayer: Boolean(itemsLayer),
        hasLabelsLayer: Boolean(labelsLayer),
      };
    });
    ...
    const anchorResult = await page.evaluate(() => {
      ...
      for (let i = 0; i < items.length && i < labels.length; i++) {
        const itemBox = items[i].getBoundingClientRect();
        const labelBox = labels[i].getBoundingClientRect();
        const id = items[i].getAttribute("data-item-id") || "unknown";
        // label center X should be near item center X (within 15px)
        const itemCenterX = itemBox.left + itemBox.width / 2;
        const labelCenterX = labelBox.left + labelBox.width / 2;
        const dx = Math.abs(itemCenterX - labelCenterX);
        if (dx > 15) {
        ...
        const aboveGap = itemBox.top - labelBox.bottom;
        const belowGap = labelBox.top - itemBox.bottom;
        const bestGap = Math.min(Math.abs(aboveGap), Math.abs(belowGap));
        if (bestGap > 35) {
