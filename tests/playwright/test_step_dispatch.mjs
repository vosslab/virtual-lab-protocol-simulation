...
async function runTests(page) {
  ...
  try {
    const incubSteps = await page.evaluate(() => {
      return PROTOCOL_STEPS.filter((s) => s.isIncubation === true).map(
        (s) => s.id,
      );
    });
    ...
    results.push({
      ...
      detail: pass
        ? "ok"
        : `got ${incubSteps.length} steps: ${incubSteps.join(", ")}`,
    });
  } catch (e) {
    results.push({
      name: "getIncubationSteps returns 3 entries {incubate_day1, incubate_48h, incubate_mtt}",
  ...
  try {
    const drugModalSteps = await page.evaluate(() => {
      return PROTOCOL_STEPS.filter(
        (s) => s.modal && s.modal.owner === "drug_treatment",
      ).map((s) => s.id);
    });
    const expectedIds = new Set([
      "carb_intermediate",
    ...
    results.push({
      ...
      detail: pass
        ? "ok"
        : `got ${drugModalSteps.length} steps: ${drugModalSteps.join(", ")}`,
    });
  } catch (e) {
    results.push({
      name: 'getModalOwnedSteps("drug_treatment") returns 4 expected steps',
  ...
  try {
    const microscopeModalSteps = await page.evaluate(() => {
      return PROTOCOL_STEPS.filter(
        (s) => s.modal && s.modal.owner === "microscope",
      ).map((s) => s.id);
    });
    const pass =
      microscopeModalSteps.length >= 1 &&
    ...
    results.push({
      ...
      detail: pass
        ? "ok"
        : `got ${microscopeModalSteps.length} steps: ${microscopeModalSteps.join(", ")}`,
    });
  } catch (e) {
    results.push({
      name: 'getModalOwnedSteps("microscope") returns >= 1 entry (count_cells)',
  ...
  try {
    const hoodSteps = await page.evaluate(() => {
      return PROTOCOL_STEPS.filter((s) => s.scene === "hood").map((s) => s.id);
    });
    ...
    results.push({
      ...
      pass,
      detail: pass
        ? `ok (${hoodSteps.length} steps)`
        : `got ${hoodSteps.length} steps`,
    });
  } catch (e) {
    results.push({
      name: 'getStepsForScene("hood") returns 14+ steps',
  ...
  try {
    const triggerStepIds = await page.evaluate(() => {
      return PROTOCOL_STEPS.filter((s) => s.completionTrigger).map((s) => s.id);
    });
    ...
    results.push({
      ...
      detail: pass
        ? "ok"
        : `missing trigger ids in set of ${triggerStepIds.length}`,
    });
  } catch (e) {
    results.push({
      name: 'getStepIdsRequiringTrigger includes "spray_hood" and "pbs_wash"',
  ...
  try {
    const checks = await page.evaluate(() => {
      const countCellsIsModal =
        PROTOCOL_STEPS.find((s) => s.id === "count_cells")?.modal !== undefined;
      const sprayHoodIsModal =
        PROTOCOL_STEPS.find((s) => s.id === "spray_hood")?.modal !== undefined;
      return {
        countCellsIsModal,
        sprayHoodIsModal,
      };
    ...
    const pass =
      checks.countCellsIsModal === true && checks.sprayHoodIsModal === false;
    results.push({
      name: 'isModalOwnedStep("count_cells") true; isModalOwnedStep("spray_hood") false',
      pass,
      detail: pass
        ? "ok"
        : `count_cells modal=${checks.countCellsIsModal}, spray_hood modal=${checks.sprayHoodIsModal}`,
    });
  } catch (e) {
    results.push({
      name: 'isModalOwnedStep("count_cells") true; isModalOwnedStep("spray_hood") false',
