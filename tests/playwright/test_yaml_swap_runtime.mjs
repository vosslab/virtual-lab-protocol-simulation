...
async function testSwappedYaml() {
	...
	try {
		...
		console.log(`  Resuspend step tools: [${resuspendStep.tool1}, ${resuspendStep.tool2}]`);

		// Verify both tools are multichannel (swapped)
		if (resuspendStep.tool1 !== 'multichannel_pipette') {
			throw new Error(`Expected tool1 to be 'multichannel_pipette', got '${resuspendStep.tool1}'`);
		}
		if (resuspendStep.tool2 !== 'multichannel_pipette') {
			throw new Error(`Expected tool2 to be 'multichannel_pipette', got '${resuspendStep.tool2}'`);
		}

		console.log('  PASS: Swapped YAML shows multichannel_pipette in interactionSequence');
		return true;
	} finally {
		await browser.close();
	}
...
async function testRestoredYaml() {
	...
	try {
		...
		if (resuspendStep.tool1 !== 'serological_pipette') {
			throw new Error(`Expected tool1 to be 'serological_pipette', got '${resuspendStep.tool1}'`);
		}
		if (resuspendStep.tool2 !== 'serological_pipette') {
			throw new Error(`Expected tool2 to be 'serological_pipette', got '${resuspendStep.tool2}'`);
		}

		console.log('  PASS: Restored YAML shows serological_pipette in interactionSequence');
		return true;
	} finally {
		await browser.close();
	}
