...
async function testDirectParentMatch() {
  ...
  try {
    ...
    const runtime_bundle_content = await fs.promises.readFile(
      runtime_bundle_path,
      "utf8",
    );
    ...
    const test_1a_result = await page.evaluate(() => {
      ...
      const parentElem = viewport?.querySelector(
        '[data-target-id="well_plate_96"]',
      );
      if (!parentElem) {
        return {
          test: "direct_parent",
          found: false,
          error: "well_plate_96 element not found",
        };
      }
    ...
    console.log(
      `[test 1a] Direct parent element check: ${JSON.stringify(test_1a_result)}`,
    );
...
async function testSubTargetInGroup() {
  ...
  try {
    ...
    const test_2a_result = await page.evaluate(() => {
      const viewport = document.querySelector('[data-testid="scene-viewport"]');
      const rowEElem = viewport?.querySelector(
        '[data-target-id="well_plate_96.row_E"]',
      );
      if (!rowEElem) {
        return {
          test: "group_element",
          found: false,
          error: "well_plate_96.row_E element not found",
        };
      }
    ...
    console.log(
      `[test 2a] Group element check: ${JSON.stringify(test_2a_result)}`,
    );
    ...
    const test_2b_result = await page.evaluate(() => {
      const viewport = document.querySelector('[data-testid="scene-viewport"]');
      const wellElem = viewport?.querySelector(
      ...
      if (!wellElem) {
        return {
          test: "well_element",
          found: false,
          error: "well_plate_96.E7 element not found",
        };
      }
    ...
    console.log(
      `[test 2b] Well element check: ${JSON.stringify(test_2b_result)}`,
    );
...
async function main() {
  try {
    ...
    console.log(
      "Validator preset hierarchy logic is wired correctly in entry.ts line 755.",
    );
  } catch (err) {
