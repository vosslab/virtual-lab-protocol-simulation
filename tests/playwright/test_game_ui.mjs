...
async function main() {
  ...
  server.on("exit", (_code) => {
    // server exited with code (not used in this smoke test)
  });
  ...
  try {
    ...
    logGate(
      "1. page_loads_no_errors",
      results.page_loads,
      pageErrors.length
        ? `pageErrors=${pageErrors.length}, console=${consoleErrors.length}`
    ...
    const sidebar = page
      .locator(
        '#sidebar, .sidebar, #protocol-checklist, [data-region="sidebar"]',
      )
      .first();
    ...
    const scene = page
      .locator("#hood-scene, #scene-stage, .scene, svg")
      .first();
    ...
    if ((await ethanolItem.count()) > 0) {
      try {
        ...
        const completedStep = page.locator(
          "li:has(#step-spray_hood).completed",
        );
    ...
    logGate(
      "8. scoring_screen_in_dom",
      results.scoring_reachable,
      results.scoring_reachable
        ? "(present, not played through)"
        : "(not in initial DOM; needs full playthrough)",
    );
    ...
    const svgInfo = await page.evaluate(() => {
      ...
      for (const svg of svgs) {
        pathCount += svg.querySelectorAll(
          "path, rect, polygon, circle, ellipse, line",
        ).length;
      }
      ...
      for (const el of all) {
        ...
        if (
          id &&
          /^(t75_flask|media_bottle|aspirating_pipette|sero_pipette|ethanol_spray|waste_container|microscope|incubator)__/.test(
            id,
          )
        ) {
          prefixedIds++;
    ...
    results.equipment_svg_renders =
      svgInfo.pathCount >= 5 && svgInfo.prefixedIds >= 1;
    logGate(
      "9. equipment_svg_renders",
      results.equipment_svg_renders,
  ...
  if (blockerCount > 0) {
    console.log(
      `BLOCKED: ${blockerCount} critical gates failed (page_loads, start_screen, scene_renders).`,
    );
    process.exit(1);
  }
  process.exit(0);
