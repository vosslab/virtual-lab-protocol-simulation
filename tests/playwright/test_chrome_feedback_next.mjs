...
async function main() {
  ...
  try {
    ...
    if (!initialHidden)
      throw new Error(
        "Test 1 failed: Feedback area should be initially hidden",
      );
    ...
    if (!feedbackVisible)
      throw new Error("Test 2 failed: Feedback area should be visible");
    ...
    if (!feedbackText)
      throw new Error("Test 2b failed: Feedback text should contain message");

    // Capture initial screenshot
    await page.screenshot({
      path: path.join(TEST_RESULTS_DIR, "1_feedback_visible.png"),
    });
    ...
    if (!feedbackHiddenAfterDismiss)
      throw new Error(
        "Test 3 failed: Feedback area should be hidden after dismiss",
      );

    // Capture after dismiss screenshot
    await page.screenshot({
      path: path.join(TEST_RESULTS_DIR, "2_feedback_dismissed.png"),
    });
    ...
    if (!nextButtonHidden)
      throw new Error("Test 4 failed: Next button should not exist initially");
    ...
    if (!nextButtonVisible)
      throw new Error("Test 5 failed: Next button should be visible");

    // Capture next button visible screenshot
    await page.screenshot({
      path: path.join(TEST_RESULTS_DIR, "3_next_button_visible.png"),
    });
    ...
    if (!clickFired)
      throw new Error("Test 6 failed: Next button click should fire handler");

    // Capture after click screenshot
    await page.screenshot({
      path: path.join(TEST_RESULTS_DIR, "4_after_next_click.png"),
    });
