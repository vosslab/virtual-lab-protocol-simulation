...
/* global PROTOCOL_STEPS, gameState */
...
async function testPipetteLiquidOverlay() {
  ...
  try {
    ...
    for (const [stepId, expectedInfo] of Object.entries(
      EXPECTED_LIQUID_STEPS,
    )) {
      ...
      while (currentStepId !== stepId && attempts < 50) {
        ...
        // This is a simplified approach: the real protocol_walkthrough_yaml.mjs handles full step navigation
        const usedItems = await page.evaluate((sid) => {
          const step =
            typeof PROTOCOL_STEPS !== "undefined"
        ...
        if (usedItems.length === 0) break;

        // Try clicking the first used item
        const itemToClick = usedItems[0];
      ...
      if (currentStepId !== stepId) {
        console.log(
          `  SKIP: could not reach step ${stepId} (at ${currentStepId})`,
        );
        continue;
      }
      ...
      if (actualColor === expectedColor) {
        console.log(
          `  OK: liquid overlay present with correct color (${actualColor})`,
        );
      } else {
        console.log(
          `  WARN: liquid color mismatch - expected ${expectedColor}, got ${actualColor}`,
        );
      }
