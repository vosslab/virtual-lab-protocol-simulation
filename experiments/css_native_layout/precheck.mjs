...
function resolvePattern(fullPattern) {
  ...
  return files.filter((f) => globRegex.test(f)).map((f) => path.join(dir, f));
}
...
async function checkClippedArtwork(page) {
  ...
  for (const placement of placements) {
    ...
    const regionElem = await placement
      .locator("..")
      .locator("..")
    ...
    const regionBox = await placement.locator("..").locator("..").boundingBox();
    ...
    const overflows = {
      ...
      right:
        placementBox.x + placementBox.width > regionBox.x + regionBox.width,
      bottom:
        placementBox.y + placementBox.height > regionBox.y + regionBox.height,
    };
    ...
    if (sides.length > 0) {
      ...
      result.push({
        ...
        placement_bbox: {
          x: placementBox.x,
          y: placementBox.y,
          w: placementBox.width,
          h: placementBox.height,
        },
        region_bbox: {
          x: regionBox.x,
          y: regionBox.y,
...
async function checkOffPageArtwork(page, viewport) {
  ...
  for (const placement of placements) {
    ...
    const corners = [
      ...
      {
        name: "top-right",
        x: placementBox.x + placementBox.width,
        y: placementBox.y,
      },
      {
        name: "bottom-left",
        x: placementBox.x,
        y: placementBox.y + placementBox.height,
      },
      {
        name: "bottom-right",
        x: placementBox.x + placementBox.width,
...
async function checkLabelLabelOverlap(page) {
  ...
  for (const label of labels) {
    ...
    const placement = await label
      .locator("..")
      .getAttribute("data-placement-name");
...
async function checkArtworkIntegrity(page, sceneContainer) {
  ...
  for (const placement of placements) {
    ...
    if (artworkBox && naturalDims.width && naturalDims.height) {
      ...
      const aspectMismatchPct = Math.abs(
        ((renderedAspect - naturalAspect) / naturalAspect) * 100,
      ).toFixed(1);
    ...
    if (artworkBox && cardBox) {
      ...
      const sidesOutside = Object.keys(artworkOutsideCard).filter(
        (s) => artworkOutsideCard[s],
      );
    ...
    if (cardBox && sceneContainer) {
      ...
      if (regionElem) {
        const regionBox = await placement
          .locator("..")
          .locator("..")
          .boundingBox();
    ...
    const clipInfo = await imgElem.evaluate((el, tolerancePx) => {
      ...
      while (
        ...
        if (clipsX || clipsY) {
          ...
          if (clipsX) {
            if (r.left > clipLeft) {
              clipLeft = r.left;
              clippedByAncestorTag =
                parent.tagName +
                (parent.className
                  ? "." + String(parent.className).split(" ").join(".")
                  : "");
            }
      ...
      return {
        ...
        visible_bbox: {
          ...
          w: Math.max(
            0,
            Math.min(imgRect.right, clipRight) -
              Math.max(imgRect.left, clipLeft),
          ),
          h: Math.max(
            0,
            Math.min(imgRect.bottom, clipBottom) -
              Math.max(imgRect.top, clipTop),
          ),
        },
        clip_over_px: {
          left: clipOverLeft,
    ...
    if (clipInfo && clipInfo.is_clipped) {
      const overflowSides = Object.keys(clipInfo.clip_over_px).filter(
        (s) => clipInfo.clip_over_px[s] > CLIP_TOLERANCE_PX,
      );
...
async function checkRegionWhitespace(page) {
  ...
  for (const region of regions) {
    ...
    const whitespacePercent =
      regionArea > 0
        ? (((regionArea - occupiedArea) / regionArea) * 100).toFixed(1)
        : 0;
...
async function checkSceneWhitespace(page) {
  ...
  const whitespacePercent =
    sceneArea > 0
      ? (((sceneArea - occupiedArea) / sceneArea) * 100).toFixed(1)
      : 0;
...
async function checkPrimaryObjectRatio(page, _viewport) {
  const container = await page.locator(".scene-container").boundingBox();
  if (!container) return null;
  ...
  if (!(await primaryPlacement.isVisible().catch(() => false))) {
    primaryPlacement = await page
      .locator('[data-region="primary_work_surface"] .placement')
      .first();
  ...
  if (!(await primaryPlacement.isVisible().catch(() => false))) {
    primaryPlacement = await page
      .locator('[data-region="work_surface"] .placement')
      .first();
  ...
  const isZoom =
    (await page.locator(".scene-container.scene-mode--detail").count()) > 0;

  const threshold = isZoom ? 70 : 25;
  const flag = ratio < threshold;

  const placementName = await primaryPlacement.getAttribute(
    "data-placement-name",
  );
...
async function checkLargestEmptyBand(page, _viewport) {
  const container = await page.locator(".scene-container").boundingBox();
  if (!container) return null;
...
async function checkSupportingDistance(page) {
  ...
  for (const placement of placements) {
    const placementName = await placement.getAttribute("data-placement-name");
    const primaryName = await primaryPlacement.getAttribute(
      "data-placement-name",
    );
    ...
    const dist = Math.sqrt(
      (centerX - primaryCenterX) ** 2 + (centerY - primaryCenterY) ** 2,
    );
  ...
  const meanNorm = (
    distances.reduce((sum, d) => sum + d.normalized, 0) / distances.length
  ).toFixed(3);
...
async function runPrecheck(
  htmlPath,
  theme = "",
  outDir = "test-results/new0_css_native/audit",
) {
  ...
  try {
    ...
    if (hasHardFail) {
      ...
      checks.label_label_overlap.length > 0 ||
      (checks.region_whitespace &&
        checks.region_whitespace.some((r) => r.flag)) ||
...
function generateMarkdownReport(sceneResults) {
  ...
  const summary = {
    ...
    scenes_pass_template: sceneResults.filter(
      (r) => r.verdict === "PASS_TEMPLATE",
    ).length,
    ...
    checks_failed: sceneResults.reduce((sum, r) => {
      ...
      if ((checks.artwork_integrity?.clipped_by_parent || []).length > 0)
        count++;
      if (
        (checks.artwork_integrity?.aspect_distorted || []).some(
          (i) => i.severity === "HARD_FAIL",
        )
      )
        count++;
      return sum + count;
  ...
  for (const result of sceneResults) {
    ...
    const primaryFlag = primaryObj
      ? primaryObj.flag
        ? "Y"
        : primaryObj.scene_mode === "template"
          ? "—"
          : "N"
      : "-";
    const sceneMode = primaryObj
      ? primaryObj.scene_mode || "composition"
      : "composition";
  ...
  for (const result of sceneResults) {
    ...
    if (checks.artwork_integrity) {
      ...
      const hasFails =
        integrity.natural_vs_rendered.filter((i) => i.severity === "WARN")
          .length > 0 ||
      ...
      if (hasFails) {
        ...
        if (
          ...
          for (const item of integrity.natural_vs_rendered.filter(
            (i) => i.severity === "WARN",
          )) {
...
function generateSizingManifest(sceneResults, _repoRoot) {
  const entries = [];
    ...
    if (integrity && integrity.natural_vs_rendered) {
      for (const item of integrity.natural_vs_rendered) {
        ...
        const entry = {
          ...
          scale_factor_used_x: item.scale_factor_w
            ? parseFloat(item.scale_factor_w)
            : undefined,
          scale_factor_used_y: item.scale_factor_h
            ? parseFloat(item.scale_factor_h)
            : undefined,
...
async function main() {
  const { pattern, outDir, __annotate, theme } = parseArgs();
  ...
  const jsonReport = {
    summary: {
      ...
      checks_failed: sceneResults.reduce((sum, r) => {
        ...
        if ((r.checks.artwork_integrity?.clipped_by_parent || []).length > 0)
          count++;
        if (
          (r.checks.artwork_integrity?.aspect_distorted || []).some(
            (i) => i.severity === "HARD_FAIL",
          )
        )
          count++;
  ...
  try {
    ...
    execSync(
      `source ${path.join(repoRoot, "source_me.sh")} && python3 ${annotateScript}`,
      {
        stdio: "inherit",
      },
    );
  ...
  console.log(
    `PASS: ${sceneResults.filter((r) => r.verdict === "PASS").length}`,
  );
  console.log(
    `PASS_TEMPLATE: ${sceneResults.filter((r) => r.verdict === "PASS_TEMPLATE").length}`,
  );
  console.log(
    `WARN: ${sceneResults.filter((r) => r.verdict === "WARN").length}`,
  );
  console.log(
    `FAIL: ${sceneResults.filter((r) => r.verdict === "FAIL").length}`,
  );
}
