...
function parseArgs() {
  ...
  console.error("Usage: node score_layout.mjs [--compare <dir_a> <dir_b>]");
  process.exit(1);
}
    ...
    if (line.startsWith("- scene_name:")) {
      ...
      for (let j = i; j < Math.min(i + 5, lines.length); j++) {
        if (lines[j].includes("class:")) {
          sceneClass = lines[j].split("class:")[1].trim().replaceAll('"', "");
          break;
        }
      }
...
function __normalizeMetric(value, min = 0, max = 100) {
...
function computeAspectRatioFidelityScore(sizingManifest, sceneName) {
  ...
  const meanMismatch =
    sceneEntries.reduce(
      (sum, e) => sum + (e.aspect_ratio_mismatch_pct || 0),
      0,
    ) / sceneEntries.length;
...
function computeSceneMetrics(sceneData, sizingManifest, sceneName) {
    ...
    checks.artwork_integrity.natural_vs_rendered.forEach((entry) => {
      if (entry.placement_name) {
        allPlacements.push({
          ...
          area:
            (entry.rendered_width_px || 0) * (entry.rendered_height_px || 0),
        });
      }
    });
  }
  ...
  const metrics = {
    ...
    region_filling: computeRegionFillingScore(checks.region_whitespace || []),
    label_readability: computeLabelReadabilityScore(checks.artwork_integrity),
...
function compareScorecards(dirA, dirB) {
  ...
  scoreA.scenes.forEach((sceneA) => {
    const sceneB = scoreB.scenes.find(
      (s) => s.scene_name === sceneA.scene_name,
    );
    if (sceneB) {
