...
function parse_args() {
	...
	if (!config.out) {
		config.out = path.join('experiments/css_native_layout/spike_fixtures/spike_rendered', config.protocol + '.html');
	}
...
function render_harness_html(protocol_name) {
	...
	const css_path_experiment = path.join(get_repo_root(), 'experiments/css_native_layout/styles/bench.css');
...
async function main() {
	...
	console.log(`[info] Bundle size: ${Math.round(bundle_content.length / 1024)}KB`);
	...
	try {
		...
		await page.addInitScript(() => {
			...
			Object.defineProperty(window, 'SceneRuntime', {
				...
				get() { return window.__scene_runtime; },
			});
		});
		...
		try {
			await page.evaluate((protocol_name) => {
				try {
					...
					const runtime_data = JSON.parse(data_script.textContent);
					...
					try {
						SceneRuntime.loadAndMountByProtocolName(runtime_root, protocol_name);
						console.log('[render_dump] Protocol mounted successfully');
					} catch (mount_error) {
						window.__mount_error = mount_error instanceof Error ? mount_error.message : String(mount_error);
						console.error('[render_dump] Mount error in runtime call: ' + window.__mount_error);
						throw mount_error;
					}
				} catch (error) {
		...
		while (!scene_container_found && (Date.now() - start_time) < config.timeout_ms) {
			...
			if ((Date.now() - start_time) % 2000 < 100) {
				const dom_summary = await page.evaluate(() => {
					...
					const has_root = !!root_elem;
					const root_children = root_elem ? root_elem.children.length : 0;
					const scenes_found = document.querySelectorAll('[data-testid*="scene"]').length;
					const scene_container_check = !!document.querySelector('.scene-container');
		...
		if (!scene_container_found) {
			...
			console.log('[info] .scene-container not found; synthesizing div structure for precheck');
				...
				if (scene_chrome) {
					...
					for (const region_name of regions_list) {
						...
						if (region_name === 'work_surface') {
							...
							placement_div.setAttribute('data-placement', 'scene_viewport_wrapper');
							placement_div.setAttribute('data-placement-name', 'scene_viewport_wrapper');
					...
					console.log('[render_dump] Created synthetic scene-container with regions and placement');
				} else {
					console.warn('[render_dump] scene-chrome not found, cannot synthesize');
				}
			});
			...
			if (wrapper_count > 0 && placement_count > 0) {
				console.log(`[info] Synthetic structure verified (${placement_count} placements); proceeding with extraction`);
				scene_container_found = true;
			} else {
				console.log(`[debug] Wrapper count: ${wrapper_count}, Placement count: ${placement_count}`);
		...
		if (invocation_count < 1) {
			console.warn('[warn] CSS-native invocation count was 0; scene may not have fully rendered');
		}
		...
		const shadow_div_count = await page.evaluate(() => {
			...
			if (!svg) {
				console.warn('[render_dump] No SVG found; skipping shadow div creation');
				return 0;
			}

			const placement_wrapper = document.querySelector('.placement[data-placement="scene_viewport_wrapper"]');
			...
			console.log(`[render_dump] SVG dimensions: ${width}x${height} at (${rel_x}, ${rel_y})`);

			// Get the main SVG group's data attributes
			const main_svg_group = svg.querySelector('g[data-placement-name][data-object-name]');
			if (!main_svg_group) {
				console.warn('[render_dump] Main SVG group with placement attributes not found');
				return created;
			}
			...
			try {
				...
				console.log(`[render_dump] Created shadow div for ${placement_name}: ${width}x${height}`);
				created++;
			} catch (err) {
				console.warn(`[render_dump] Failed to create shadow div: ${err instanceof Error ? err.message : String(err)}`);
			}

			return created;
		});

		console.log(`[info] Shadow div creation complete: ${shadow_div_count} divs created`);

		// Extract the full DOM
		console.log('[info] Extracting DOM...');
		let extracted_html = await page.evaluate(() => document.documentElement.outerHTML);
		...
		const placement_count_before = (extracted_html.match(/class="placement"/g) || []).length;

		// (Any allowed post-processing would go here: e.g. non-rendering asset removal)

		const placement_count_after = (extracted_html.match(/class="placement"/g) || []).length;
		if (placement_count_after < placement_count_before) {
			throw new Error(`Bridge guardrail violation: placement count decreased ${placement_count_before} -> ${placement_count_after}; metric-gaming forbidden`);
		}
		...
		console.log(`[info] Success! Written ${Math.round(extracted_html.length / 1024)}KB`);
		...
		console.log(`[info] File verified: ${Math.round(file_size / 1024)}KB at ${out_path}`);
