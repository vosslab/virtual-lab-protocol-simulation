...
const TEMPLATES_DIR = path.join(REPO_ROOT, "experiments/css_native_layout/templates");
const OUT_DIR_ASSETS = path.join(REPO_ROOT, "docs/active_plans/new2_no_crop_audit_assets");
...
function classify(metrics) {
	...
	const ovBottom = metrics.img_y + metrics.img_h - (metrics.placement_y + metrics.placement_h);
	const ovLeft = metrics.placement_x - metrics.img_x;
	const ovRight = metrics.img_x + metrics.img_w - (metrics.placement_x + metrics.placement_w);
	const sides = [];
	...
	if (metrics.natural_w > 0 && metrics.natural_h > 0 && metrics.img_w > 0 && metrics.img_h > 0) {
		...
		if (aspectPct > ASPECT_DISTORTION_PCT) {
			issues.push({ type: "distorted", aspect_mismatch_pct: Math.round(aspectPct * 10) / 10 });
		}
	}
	...
	if (metrics.natural_w > 0 && metrics.natural_h > 0) {
		...
		if (areaPct < TINY_AREA_PCT) {
			issues.push({ type: "too-small", area_pct: Math.round(areaPct * 10) / 10 });
		}
	}
...
async function inspectScene(page, scene) {
	...
	const data = await page.evaluate(() => {
		...
		for (const p of placements) {
			const pname = p.getAttribute("data-placement-name") || p.getAttribute("data-placement") || "";
...
async function main() {
	if (!fs.existsSync(OUT_DIR_ASSETS)) fs.mkdirSync(OUT_DIR_ASSETS, { recursive: true });
	...
	for (const scene of SCENES) {
		...
		console.log(`  placements=${r.total_placements} violations=${r.violations.length}`);
		results.push(r);
	}
	await browser.close();
